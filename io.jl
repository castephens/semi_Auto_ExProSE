...
#christina - write a text file for PCs (coords vs magnitude)
function writepcchristina(out_filepath::AbstractString,
                    pca::PCA,
                    atoms::Array{Atom,1},
                    pc::Integer;
                    weighting::Real=50.0,
    ...
    @assert size(pca.evecs, 1) == 3 * length(inds_to_use) "PCA and atom list have different sizes"
    disps = pca.evecs[:,pc]
    #println(length(disps))     #christina
    open(expanded_path, "w") do out_file
        for (i, j) in enumerate(inds_to_use)
            pc_coords = atoms[j].coords + disps[3i-2:3i] * weighting
	    println(out_file, round(atoms[j].coords[1], digits=2), " ", round(atoms[j].coords[2], digits=2), " ", round(atoms[j].coords[3], digits=2))
	    println(out_file, round(pc_coords[1], digits=2), " ", round(pc_coords[2], digits=2), " ", round(pc_coords[3], digits=2)) 
        end
    end
end
...
function writepcviews(out_prefix::AbstractString,
	...
	out_filepath2 = "$expanded_prefix$i.txt" #christina
        writepcview(out_filepath, pca, atoms, i)
	writepcchristina(out_filepath2, pca, atoms, i) #christina
    end
...
#Christina
"""
function readpocketweight(pdb_filepath::AbstractString)
    expanded_path = expanduser(pdb_filepath)
    @assert isfile(expanded_path) "Not a valid filepath: \"$expanded_path\""
    pock_points = Dict{Int, Array{Float64}}()
    counter = 0
    open(expanded_path, "r") do pdb_file
        for line in eachline(pdb_file)
            if startswith(line, "ATOM  ") || startswith(line, "HETATM")
                counter += 1
                weight = parse(Float64, line[21:23])
                pocket_n = parse(Int, line[23:26])
                if haskey(pock_points, pocket_n)
                    pock_points[pocket_n] = hcat(pock_points[pocket_n], [weight])
                else
                    pock_points[pocket_n] = [weight]
                end
            end
        end
    end
